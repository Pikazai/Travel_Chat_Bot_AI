...
# Full-featured Streamlit travel chatbot
# Features:
# - OpenAI-based bilingual city/date extraction
# - OpenWeatherMap forecast
# - Hybrid restaurants: Google Places API + CSV fallback
# - Food AI Assistant: CSV local foods + GPT fallback
# - Images from Pixabay API (stable, no redirect)
# - Geocoding via Nominatim + pydeck map
# - Logging to SQLite + simple analytics
#
# Requirements:
# pip install streamlit openai requests geopy pandas pydeck sqlite3
# Place API keys in .streamlit/secrets.toml (see README in code comments)
...
# -------------------------
# CONFIG / SECRETS (expect in .streamlit/secrets.toml)
# -------------------------
...
def show_map(lat, lon, zoom=8, title=""):
    ...
    # 💡 Dùng bản đồ sáng miễn phí từ Carto
    deck = pdk.Deck(
        layers=[layer],
        initial_view_state=view,
        map_provider=None,
        map_style="https://basemaps.cartocdn.com/gl/positron-gl-style/style.json",  # 🌤 sáng, nhẹ, miễn phí
...
def get_pixabay_image(query, per_page=3):
    """
    Lấy ảnh minh họa từ Pixabay theo từ khóa.
...
def get_city_image(city):
    """
    Lấy ảnh đại diện cho thành phố từ Pixabay.
    """
    if not city:
        return None
    queries = [
...
def get_food_images(food_list):
    """
    Lấy ảnh minh họa món ăn từ Pixabay (top 5 kết quả).
    """
    images = []
...
def get_local_foods(city):
    try:
        ...
        # Support formats: city,foods  (foods comma-separated) OR city,food1,food2,...
        ...
        if not row.empty:
            ...
            if "foods" in row0.index:
                ...
                # other columns: first is city, rest are foods
...
# -------------------------
# SUGGESTIONS / ITINERARY / COST / PHOTOSPOTS
# -------------------------
def suggest_trip_plan(city, days=3):
    return f"📅 **Lịch trình gợi ý {days} ngày tại {city}:**\\n- Ngày 1: Khám phá trung tâm, chợ, di tích.\\n- Ngày 2: Tham quan thiên nhiên, thưởng thức ẩm thực.\\n- Ngày 3: Mua sắm, chụp ảnh, nghỉ ngơi."
...
with st.sidebar:
    ...
    st.markdown("---")
    st.write("Secrets should be stored in .streamlit/secrets.toml")
...
if user_input:
    ...
    #     blocks.append(suggest_trip_plan(city_guess))
    ...
    with cols[0]:
        if "Map" in info_options:
            show_map(lat, lon, title=addr or city_guess)
    ...
    with cols[1]:
        if "Food" in info_options:
            ...
            # food dishes (CSV or GPT fallback)
