...
Embed câu hỏi (text-embedding-3-small),
...
Chạy thêm một bản patch nhỏ để highlight metadata nguồn trong UI (ví dụ hiển thị danh sách sources bên cạnh assistant message).
Tạo một script seed CSV mẫu (data/vietnam_travel_docs.csv) với vài dòng demo (mình sẽ tạo file sẵn để bạn thử RAG).

Thêm một command-line script seed_vietnam_rag.py để dễ seed nhiều lần