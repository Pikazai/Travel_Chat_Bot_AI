...
def init_chroma():
    ...
    # Determine expected embedding dimension from the embedding model we will use
    # text-embedding-3-small -> dimension 1536 (as used by OpenAI's new embeddings)
    EXPECTED_DIM = 1536
    # travel_col = safe_get_collection(chroma_client, "vietnam_travel", expected_dim=EXPECTED_DIM)
    # memory_col = safe_get_collection(chroma_client, "chat_memory", expected_dim=EXPECTED_DIM)
    # intent_col = safe_get_collection(chroma_client, "intent_bank", expected_dim=EXPECTED_DIM)
    travel_col = safe_get_collection(chroma_client, "vietnam_travel_v2", expected_dim=1536)
    memory_col = safe_get_collection(chroma_client, "chat_memory_v2", expected_dim=1536)
    intent_col = safe_get_collection(chroma_client, "intent_bank_v2", expected_dim=1536)

    # --- Debug info ---
    abs_dir = os.path.abspath(CHROMA_PERSIST_DIR)
    print(f"📁 Chroma đang lưu dữ liệu tại: {abs_dir}")
